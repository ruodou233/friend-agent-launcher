...
`--assets-dir` 和 `--output-dir` 必须都在源码仓库外。构建器拒绝覆盖已有输出，避免把第三方二进制或朋友个人配置误纳入 Git 历史。
...
安装脚本会在目标系统核验官方签名、发布者或 Team ID；构建时只计算内容哈希。某个第三方 Endpoint/Key 能否工作仍取决于协议兼容性、模型权限、余额与网络可达性。官方客户端升级也可能要求新的离线 Runtime，旧模板不能永久替代兼容性测试。

公开仓库不发布预构建二进制。你可以在自己的受信环境构建私人测试包，但不要把含第三方二进制或个人配置的产物直接提交回源码仓。如果还要对外分发生成的 ZIP，须先逐项确认载荷的再分发权，并保留包内 `THIRD_PARTY-NOTICES.md` 与上游许可文本。
